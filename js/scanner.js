class ScannerController{
  constructor(){this.scanner=null;this.locked=false;}
  async start(onScan){
    if(this.scanner)return;
    if(typeof Html5Qrcode==="undefined")throw new Error("No se cargó la biblioteca del escáner. Revise la conexión.");
  ...
  friendlyError(e){
    ...
    if(/NotAllowed|Permission|denied/i.test(m))return "Permiso de cámara denegado. Abra la configuración del sitio y permita la cámara.";
    if(/NotFound|camera/i.test(m))return "No se encontró una cámara disponible.";
    return m;
  }
}
